...
class LitomiImageProvider extends VioletImageProvider {
  ImageList imageList;
  String id;

  LitomiImageProvider(this.imageList, this.id);
  ...
  Future<Map<String, String>> getHeader(int page) async {
    ...
    return await ScriptManager.runLitomiGetHeaderContent(id);
  }
  ...
  Future<double> getEstimatedImageHeight(int page, double baseWidth) async {
    ...
    if (_estimatedCache == null) {
      _estimatedCache = List<double>.filled(
        imageList.smallThumbnails!.length,
        0,
      );
  ...
  /*
    https://ltn.gold-usergeneratedcontent.net/common.js의 get_gg함수는 30분에 한 번씩 호출된다.
