...
class SeriesFinder {
  static Future<void> doFind1() async {
    ...
    for (var i = 0; i < artists.length; i++) {
      ...
      ).toList().where((element) => element.length > 1).toList().forEach((
        element,
      ) {
  ...
  static Future<void> doFind2() async {
    ...
    var seriesList = <(String, List<QueryResult>)>[];
    ...
    for (var i = 0; i < artists.length; i++) {
      ...
      HitomiTitleCluster.doClustering(
        kv.value.map((e) => qr[e]).map((e) => e.title() as String).toList(),
      ).toList().where((element) => element.length > 1).toList().forEach((
        element,
      ) {
        seriesList.add((
          'artist:${kv.key}',
          element.map((e) => qr[kv.value[e]]).toList(),
        ));
      });
    }
    ...
    seriesList.sort((x, y) => x.$2.length.compareTo(y.$2.length));
    for (var element in seriesList) {
      print('[${element.$2.length}] ${element.$1}');
    }
  }
}
